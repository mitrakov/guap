          ...
          child: Builder(builder: (context) => ElevatedButton(
            ...
            onPressed: () {
              if (ctrl1.text.isEmpty || ctrl2.text.isEmpty)
                ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Empty passcode!")));
              else if (ctrl1.text == ctrl2.text)
                setPasscode(ctrl1.text).then((b) => Navigator.popAndPushNamed(context, "/main"));
              else ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Passcode fields differ!")));
            }
          ))
        ),
      ]
